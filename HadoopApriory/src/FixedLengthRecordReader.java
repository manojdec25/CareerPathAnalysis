 ...
 * uses the record length property set within the FixedLengthInputFormat to read
 * one record at a time from the given InputSplit. This record reader does not
 * support compressed files.<BR>
 * <BR>
 * 
 * Each call to nextKeyValue() updates the LongWritable KEY and Text VALUE.<BR>
 * <BR>
 * 
 ...
 * 
 */
public class FixedLengthRecordReader implements
	...
	public FixedLengthRecordReader(InputSplit inputSplit, JobConf conf)
			throws IOException{
	...
	@Override
	public boolean next(LongWritable recordKey, Text recordValue)
			throws IOException {
		if (recordKey == null) {
		...
		if (currentPosition < splitEnd) {
			...
			while (totalRead != recordLength) {
				...
				totalRead += read;
				totalToRead -= read;
				// LOG.info("READ: just read=" + read +" totalRead=" + totalRead
			...
			// LOG.info("VALUE=|"+fileInputStream.getPos()+"|"+currentPosition+"|"+splitEnd+"|"
			// + recordLength + "|"+recordValue.toString());
